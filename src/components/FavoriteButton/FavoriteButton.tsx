...
const FavoriteButton = ({game , className} : {game : gameType , className? : string}) => {
    return (
            <svg width="30" height="30" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg" className={classNames(className!,style.button)}
                 onClick={async () => {
                     try {
                         const games = await localStoreGetItem("games");
                         const updatedGames = games ? [...games, game] : [game];
                         await localStoreSetItem("games", updatedGames);

                         // После обновления данных, вы можете получить свежие данные
                         const freshGames = await localStoreGetItem("games");
                         console.log("Fresh Games:", freshGames);
                     } catch (error) {
